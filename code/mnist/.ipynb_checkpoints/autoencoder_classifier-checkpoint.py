...
# def train_frozen_encoder_classifier(encoder, classifier, train_loader, num_epochs=10, learning_rate=0.001):
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#     encoder.to(device)
#     classifier.to(device)

#     criterion = nn.CrossEntropyLoss()
#     optimizer = optim.Adam(classifier.parameters(), lr=learning_rate)  # Only optimize classifier
...
#             with torch.no_grad():  # Prevent gradients from updating the encoder
#                 latent_vectors = encoder(images)

#             outputs = classifier(latent_vectors)  # Train only the classifier
...
# def evaluate_frozen_classifier(encoder, classifier, test_loader):
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#     encoder.to(device)
#     classifier.to(device)
#     classifier.eval()
...
#             images, labels = images.to(device), labels.to(device)
#             latent_vectors = encoder(images)  # Get latent representations
...
#     pretrained_encoder.load_state_dict(torch.load(artifacts_dir / "encoder_mnist.pth"))  # Load trained weights
#     pretrained_encoder.eval()  # Set to evaluation mode

#     # Freeze the encoder
#     for param in pretrained_encoder.parameters():
#         param.requires_grad = False  # Ensure it is not updated during training
...
#     # Initialize classifier and train it on frozen encoder representations
#     classifier = Classifier()
#     train_frozen_encoder_classifier(pretrained_encoder, classifier, train_loader, num_epochs=10)

#     # Evaluate classifier
#     evaluate_frozen_classifier(pretrained_encoder, classifier, test_loader)

#     # Show 50 predictions in a grid
#     visualize_many_predictions(pretrained_encoder, classifier, test_loader, artifacts_dir, num_images=50, images_per_row=10)
