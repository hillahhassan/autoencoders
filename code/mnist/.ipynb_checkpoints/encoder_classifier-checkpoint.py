...
#         self.encoder = Encoder(latent_dim)  # Not pre-trained
...
# def train_encoder_and_classifier(model, train_loader, num_epochs=10, learning_rate=0.001):
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#     model.to(device)

#     criterion = nn.CrossEntropyLoss()
#     optimizer = optim.Adam(model.parameters(), lr=learning_rate)  # Optimizing encoder + classifier
...
#             loss.backward()  # Backpropagate through encoder & classifier
...
# def plot_training_metrics(train_losses, train_accuracies, artifacts_dir):
...
#     plt.savefig(artifacts_dir / "training_metrics.png")
...
#     train_dataset = datasets.MNIST(root="/datasets/cv_datasets/data", train=True, transform=transform, download=True)
...
#     train_losses, train_accuracies = train_encoder_and_classifier(classification_model, train_loader, num_epochs=10)
#     torch.save(classification_model.state_dict(), artifacts_dir / "encoder_classifier_mnist.pth")

#     # Plot training loss & accuracy
#     plot_training_metrics(train_losses, train_accuracies, artifacts_dir)

#     # Evaluate model
#     evaluate_joint_model(classification_model, test_loader)

#     # Show predictions
#     visualize_joint_predictions(classification_model, test_loader, artifacts_dir)
...
#     # Load trained model
#     model_path = artifacts_dir / "encoder_classifier_mnist.pth"
...
#     # Save the plots to artifacts directory
#     shutil.copy("latent_tsne.png", artifacts_dir / "latent_tsne_encoder_classifier.png")
#     shutil.copy("image_tsne.png", artifacts_dir / "image_tsne_encoder_classifier.png")
